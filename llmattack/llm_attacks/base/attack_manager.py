...
class MultiPromptAttack(object):
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                ...
                if log_mode == "multi":
                    self.log_multi_queries(self.control_str, best_loss)
                elif log_mode == "indiv":
                    if i == n_steps-1:
                        self.log_indiv_query(step_d = self.step_d)
                else:
                    raise ValueError("Log Mode is wrong")
    ...
    def log_indiv_query(self, step_d = None):
    ...
    def log_multi_queries(self, control, loss):
        with open(self.logfile, 'r') as f:
            log = json.load(f)
...
class ModelWorker(object):
    ...
    def __init__(self, model_path, model_kwargs, tokenizer, conv_template, device):
        ...
        # from PR:https://github.com/llm-attacks/llm-attacks/pull/77/commits/32f8f84616e041621e43acc95983dbeb4d6905e4
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        ...
        if 'oasst-sft-6-llama-30b' in params.tokenizer_paths[i]:
            ...
            print("if 'oasst-sft-6-llama-30b' in params.tokenizer_paths[i]:")
        ...
        if 'falcon' in params.tokenizer_paths[i]:
            tokenizer.padding_side = 'left'
            print("if 'falcon' in params.tokenizer_paths[i]:")
        if "vicuna" in params.tokenizer_paths[i]:
            tokenizer.padding_side = 'left'
